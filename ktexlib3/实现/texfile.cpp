...
bool ktexlib::v3::gen_bc3universal(const wchar_t* path)
{
	...
	for (size_t i = 0; i < mipscount; i++)
	{
		...
		size_t length = (dx.height * dx.rowPitch * DirectX::BitsPerPixel(DXGI_FORMAT_BC3_UNORM));//BC3(DXT5)平均每像素一字节
...
void ktexlib::v3detail::Ktex::SaveFile(std::filesystem::path path)
{
	//验证mipmap链
	{
		...
		for (auto it = mips.begin() + 1; it != mips.end(); it++)
		{
			if (it->width != width / 2 || it->height != height / 2)
